...
import org.kaloz.excercise.marsrover.NasaHQ.{RegisterRover, RoverRegistered}
import org.kaloz.excercise.marsrover.api.Action._
...
with OneInstancePerTest {
  ...
  "NasaHQ" should {
    "subscribe registration topic" in {
      ...
      EventFilter.info(start = "Subscribed to registration topic!", occurrences = 1) intercept {
        TestActorRef(new NasaHQ(MarsExpeditionConfiguration(PlateauConfiguration(5,5), List(RoverConfiguration(RoverPosition(1, 2, N), List(L)), RoverConfiguration(RoverPosition(3, 3, E), List(M))))))
      }
    }
  ...
  private trait scope {
    ...
    val nasaHQ = TestActorRef(new NasaHQ(MarsExpeditionConfiguration(PlateauConfiguration(5,5), List(RoverConfiguration(RoverPosition(1, 2, N), List(L)), RoverConfiguration(RoverPosition(3, 3, E), List(M))))))
